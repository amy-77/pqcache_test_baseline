...
class GPUCacheManager:
    ...
    # 添加: 新token自动替换最久远的token，
    def add_new_token(self, new_key, new_value, layer_idx):
        layer_idx = layer_idx % self.layer_cnt
        assert new_key.shape == (self.bsz, self.n_kv_head, 1, self.dim), new_key.shape
        # 获取最久远的token
        # 在这个循环队列实现中，"最久远的token"的物理位置会不断变化
        # 循环队列的优势：
        # 高效的方式：直接替换，无需移动其他元素
        # buffer[local_to_evict_idx] = new_token
        # local_to_evict_idx = (local_to_evict_idx + 1) % local_size
        ...
        # 将最久远的token 拷贝到cpu_key_buffers和cpu_value_buffer中
    ...
    def fetch_and_concat_kv_w_cache(self, indices:torch.Tensor, layer_idx):
        ...
        if self.global_cache_size > 0:
            ...
            self.k.view([self.n_kv_head * self.total_budget, self.dim]).scatter_(-2, hit_scatter_index.unsqueeze(-1).expand_as(selected_global_key), selected_global_key)
            self.v.view([self.n_kv_head * self.total_budget, self.dim]).scatter_(-2, hit_scatter_index.unsqueeze(-1).expand_as(selected_global_value), selected_global_value)
            ...
            self.k.view([self.n_kv_head * self.total_budget, self.dim]).scatter_(-2, miss_scatter_index.unsqueeze(-1).expand_as(fetch_global_key), fetch_global_key)
            self.v.view([self.n_kv_head * self.total_budget, self.dim]).scatter_(-2, miss_scatter_index.unsqueeze(-1).expand_as(fetch_global_value), fetch_global_value)

...
def calc_recall(query, key, dummy_topk_indices, num_kv_group, topk_size):
    ...
    # 步骤1.2: 计算近似 dummy top-k
    if dummy_topk_indices.shape[1] != real_topk_indices.shape[1]:
        dummy_topk_indices = repeat(dummy_topk_indices, num_kv_group, 1) # [bsz, n_heads, q_len, topk]
    ...
    # 步骤3: 比较PQ选择的top-k和真实top-k的交集
    for i in range(dummy_topk_indices.shape[0]):
        dummy = dummy_topk_indices[i,0,:]  # PQ选择的top-k
        real = real_topk_indices[i,0,:]  # 真实top-k

...
def parse_args(args=None):
    ...
    parser.add_argument('--enable_neff_analysis', action='store_true',
                        help='Enable Neff analysis for attention sparsity')
    parser.add_argument('--neff_samples', type=int, default=None,
                        help='Number of samples for Neff analysis')
...
def build_chat(tokenizer, prompt, model_name):
    ...
    if "chatglm3" in model_name:
        ...
        if "3" in model_name:
            ...
            # 使用apply_chat_template: 将对话消息列表转换为模型期望的特定格式，添加特殊token，自动添加模型需要的特殊标记（如 <|begin_of_text|>, <|start_header_id|> 等）
...
def get_pred(args, model, tokenizer, rank, world_size, data, max_length, max_gen, prompt_format, dataset, model_name, model2path, out_path):
    ...
    # print(f"dataset: {dataset}")  #qasper 
    print(f"model_name: {model_name}")  #llama-3.1
    print(f"model2path: {model2path}")
    print(f"out_path: {out_path}") #pred/llama-3.1/qasper/pq_4321_rerun_v0/budget_0.1_rec_0.5_sink_32_mode_pq_search_gqa_True_subvec_2_subbit_6_max_iter_0.jsonl
    print("=" * 60)
    ...
    for i, json_obj in tqdm(enumerate(data)):
        ...
        # 将prompt模板 和 实际数据 组合成完整的提示词   prompt_format 是 dataset2prompt[dataset]
        prompt = prompt_format.format(**json_obj)
        if i == 0:
            print(f"prompt: {prompt}")
        
        # 将文本转换成token ids，取第一个序列的token ids
        ...
        if i == 0:
            print(f"original_token_cnt: {original_token_cnt}")
        ...
        if dataset not in ["trec", "triviaqa", "samsum", "lsht", "lcc", "repobench-p"]:
            ...
            if i == 0:
                print(f"build_chat ---prompt: {prompt}")
        ...
        if dataset == "samsum":
            ...
            if i == 0:
                print(f"model.generate ---input.input_ids: {input.input_ids}")
                print(f"model.generate ---input.attention_mask: {input.attention_mask}")
                print(f"model.generate ---tokenizer.eos_token_id: {tokenizer.eos_token_id}")
                print(f"model.generate ---max_gen: {max_gen}")
        ...
        # 将模型生成的数字token序列 转换 回人类可读的文本
        pred = tokenizer.decode(output[context_length:], skip_special_tokens=True) # 只解码生成的token序列
        ...
        if i == 0:
            print(f"post_process ---pred: {pred}")
...
def load_model_and_tokenizer(args, path, model_name, device, pp_size = 1):
    if "chatglm" in model_name or "internlm" in model_name or "xgen" in model_name:
        ...
        config.gqa = (args.gqa == "True")
        config.max_iter = args.max_iter
        config.device = torch.device("cuda:1")  # 使用CUDA_VISIBLE_DEVICES映射的第一个GPU
        config.mean_v_trick = (args.sparq_mean_v_trick == "True")
        config.recent_ratio = args.recent_ratio
        # 添加Neff分析配置
        ...
        if config.compressor == "pq_search":
            config.max_seq_len = 50000
            config.cache_block_size = 128
            config.global_cache_size = 4096
            config.cache_topk = 32
            #调用 pqsearch.py 的initialize_objects函数
            initialize_objects(config, model=model_name)
...
if __name__ == '__main__':
    ...
    device = torch.device("cuda:1")  # 使用CUDA_VISIBLE_DEVICES映射的第一个GPU
